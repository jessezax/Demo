...
void add_record(MYSQL *pConn)
{
  ...
  // 强大的sprintf为用户减少输入 ，避免让用户输入完整的命令，而只需要记录的数据输入
   sprintf(cmd, "insert into quintuple(SourceIp,SourcePort,DestinIp,DestinPort,protocol) values('%s','%s','%s','%s','%s')",quintuples.SourceIp,quintuples.SourcePort,quintuples.DestinIp,quintuples.DestinPort,quintuples.protocol);
...
void mod_record(MYSQL *pConn,int id)
{
  ...
  MYSQL_RES *pRes=NULL;
  char cmd[1024] = {0};
  tuples quintuples={"192.168.0.1","1","192.168.80.0.2","2","UDP"};
  sprintf(cmd,"update quintuple  set SourceIp='%s',SourcePort='%s',DestinIp='%s',DestinPort='%s',protocol='%s' where id=%d ",quintuples.SourceIp,quintuples.SourcePort,quintuples.DestinIp,quintuples.DestinPort,quintuples.protocol,id);
    ret = mysql_real_query(pConn, cmd, (unsigned int)strlen(cmd));
  if(! ret) // ret = 0是表示正确
  {
    // 打印受到影响的行的个数
